...
<div class="max-w-7xl mx-auto px-6 py-10 space-y-8">
    ...
    <div>
        <h1 class="text-2xl font-bold text-gray-900">
            Riwayat Stok – {{ $item->name }} <br> Cabang {{ $stock->warehouse->cabangResto->name ?? '-' }} <br> Gudang {{ $stock->warehouse->name }}
        </h1>
        <p class="text-gray-600 mt-1 text-sm">
            Riwayat perubahan stok lintas seluruh cabang dan gudang.
        </p>
    </div>
    ...
    <form method="GET"
            ...
            <select name="user"
                ...
                @foreach ($users as $u)
                    <option value="{{ $u->username }}"
                        @selected(request('user') == $u->username)>
                        {{ $u->username }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-span-full flex justify-end pt-2 gap-2">
            <button
                class="bg-emerald-600 text-white px-6 py-2 rounded-lg shadow
            ...
            class="bg-gray-100 px-6 py-2 rounded-lg shadow
