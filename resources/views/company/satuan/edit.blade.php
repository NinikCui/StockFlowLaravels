...
<main class="max-w-4xl mx-auto px-6 py-10">
    ...
    <div class="bg-white shadow-sm border border-gray-100 rounded-2xl p-6">
        ...
        <form action="{{ route('satuan.update', [$companyCode, $satuan->code]) }}"
              method="POST"
              class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="is_active"
                        class="w-full rounded-lg border-gray-300 px-4 py-2 focus:ring-blue-300 focus:border-blue-500">
                    <option value="1" {{ $satuan->is_active ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ !$satuan->is_active ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
