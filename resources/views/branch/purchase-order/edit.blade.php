...
<div class="max-w-5xl mx-auto px-6 py-8">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Purchase Order Cabang</h1>
    ...
    <form method="POST" action="{{ route('branch.po.update', [$branchCode, $po->id]) }}" id="poForm">
        @csrf
        @method('PUT')

        {{-- INFO --}}
        <div class="bg-white p-6 rounded-xl border shadow-sm mb-6">
            <h2 class="text-lg font-semibold mb-4">Informasi PO</h2>

            <div class="grid grid-cols-3 gap-4">

                <div>
                    <label class="text-sm font-medium">Cabang</label>
                    <input readonly value="{{ $po->cabangResto->name }}"
                        class="mt-1 p-2 w-full bg-gray-100 border rounded-lg">
                </div>

                <div>
                    <label class="text-sm font-medium">Gudang</label>
                    <input readonly value="{{ $po->warehouse->name }}"
                        class="mt-1 p-2 w-full bg-gray-100 border rounded-lg">
                </div>

                <div>
                    <label class="text-sm font-medium">Supplier</label>
                    <input readonly value="{{ $po->supplier->name }}"
                        class="mt-1 p-2 w-full bg-gray-100 border rounded-lg">
                </div>

                <div>
                    <label class="text-sm font-medium">Tanggal PO</label>
                    <input readonly value="{{ $po->po_date }}"
                        class="mt-1 p-2 w-full bg-gray-100 border rounded-lg">
                </div>

                <div>
                    <label class="text-sm font-medium">Tanggal Diharapkan</label>
                    <input readonly value="{{ $po->expected_delivery_date }}"
                        class="mt-1 p-2 w-full bg-gray-100 border rounded-lg">
                </div>
        ...
        <div class="bg-white p-6 rounded-xl border shadow-sm">
            <h2 class="text-lg font-semibold mb-4">Item Pembelian</h2>
...
function addRow(data = {}) {
    ...
    const qty = data.qty ?? (selected?.min_order_qty ?? 1);
    const price = data.price ?? (selected?.price ?? 0);
...
function changeItem(select, index) {
    ...
    if (find) {
        row.querySelector(".qtyInput").value = find.min_order_qty;
        row.querySelector(".priceInput").value = find.price;
    } else {
